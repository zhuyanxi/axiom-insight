...
// dashboard package must never import Analyzer, AST, plugin transport or
// generated-document YAML contracts, transitively.
func TestPackageImportBoundary(t *testing.T) {
	config := &packages.Config{
		Mode: packages.NeedName | packages.NeedImports | packages.NeedDeps,
	}
	pkgs, err := packages.Load(config, "github.com/zhuyanxi/axiom-insight/dashboard")
	if err != nil {
	...
	forbiddenExact := []string{
		...
		"github.com/zhuyanxi/axiom-insight/generator", // the generated-document YAML contract package
	}
	...
	visit = func(pkg *packages.Package) {
		...
		for _, forbiddenPath := range forbiddenExact {
			if pkg.PkgPath == forbiddenPath {
				t.Errorf("dashboard transitively imports forbidden package %q", pkg.PkgPath)
			}
		}
