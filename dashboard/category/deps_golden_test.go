...
func currentDepsGolden(t *testing.T) categoryGolden {
	t.Helper()
...
// TestDepsGolden fixes the deterministic P2-09 output bytes: Database,
// Cache and client rows, panel shapes, rendered PromQL, trace links and
// diagnostics. Regenerate with SI_UPDATE_GOLDEN=1.
func TestDepsGolden(t *testing.T) {
	contents, err := json.MarshalIndent(currentDepsGolden(t), "", "  ")
	if err != nil {
		t.Fatalf("marshal golden: %v", err)
	}
	contents = append(contents, '\n')

	goldenPath := filepath.Join("testdata", "deps_golden.json")
	...
	if string(contents) != string(expected) {
		t.Fatalf("dependency rows differ from golden %s; set %s=1 to regenerate", goldenPath, updateSnapshotEnv)
	}
}
