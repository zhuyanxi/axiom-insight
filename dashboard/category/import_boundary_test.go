...
// TestImportBoundary is the negative compile boundary check: the category
// package must never transitively import Analyzer, AST, plugin transport
// or generated-document YAML contracts.
func TestImportBoundary(t *testing.T) {
	config := &packages.Config{
		Mode: packages.NeedName | packages.NeedImports | packages.NeedDeps,
	}
	pkgs, err := packages.Load(config, "github.com/zhuyanxi/axiom-insight/dashboard/category")
	if err != nil {
	...
	visit = func(pkg *packages.Package) {
		...
		for _, forbiddenPath := range forbiddenExact {
			if pkg.PkgPath == forbiddenPath {
				t.Errorf("category transitively imports forbidden package %q", pkg.PkgPath)
			}
		}
