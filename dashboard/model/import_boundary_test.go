...
// model package must never import the analyzer, AST, plugin transport,
// generator or IR packages, transitively.
func TestPackageImportBoundary(t *testing.T) {
	config := &packages.Config{
		Mode: packages.NeedName | packages.NeedImports | packages.NeedDeps,
	}
	pkgs, err := packages.Load(config, "github.com/zhuyanxi/axiom-insight/dashboard/model")
	if err != nil {
	...
	forbiddenExact := []string{
		...
		"github.com/zhuyanxi/axiom-insight/ir/v1",
	...
	visit = func(pkg *packages.Package) {
		...
		for _, forbiddenPath := range forbiddenExact {
			if pkg.PkgPath == forbiddenPath {
				t.Errorf("dashboard/model transitively imports forbidden package %q", pkg.PkgPath)
			}
		}
