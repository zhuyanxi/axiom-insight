...
// logging planner must never import Analyzer, AST, plugin transport or
// generated-document packages, transitively.
func TestPackageImportBoundary(t *testing.T) {
	config := &packages.Config{
		Mode: packages.NeedName | packages.NeedImports | packages.NeedDeps,
	}
	pkgs, err := packages.Load(config, "github.com/zhuyanxi/axiom-insight/generator/planner/logging")
	if err != nil {
	...
	visit = func(pkg *packages.Package) {
		...
		for _, forbiddenPath := range forbiddenExact {
			if pkg.PkgPath == forbiddenPath {
				t.Errorf("generator/planner/logging transitively imports forbidden package %q", pkg.PkgPath)
			}
		}
