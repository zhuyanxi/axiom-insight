...
// docs/05-generation-config.md: blocks before the "无效示例" heading must
// strictly decode and resolve; blocks after it must fail with a config
// error that carries the generation path.
func TestDocumentationConfigExamples(t *testing.T) {
	...
	for index, block := range validBlocks {
		config, err := DecodeConfigFile([]byte(block))
		if err != nil {
			t.Errorf("valid block %d does not decode strictly: %v", index+1, err)
			continue
		}
		if config == nil {
			t.Errorf("valid block %d has no generation node", index+1)
			continue
		}
	...
	for index, block := range invalidBlocks {
		config, decodeErr := DecodeGeneration([]byte(block))
		...
		if !strings.Contains(message, "generation") {
			t.Errorf("invalid block %d error lacks generation context: %q", index+1, message)
		}
	}
}
...
func readConfigurationDocument(t *testing.T) string {
	t.Helper()
	path := filepath.Join("..", "..", "docs", "05-generation-config.md")
